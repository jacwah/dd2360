...
/* matmul kernel with shared memory */
__global__
void global_sgemm_kernel(float *C, float *A, float *B, long size)
{
	const long col = blockIdx.x * blockDim.x + threadIdx.x;
	const long row = blockIdx.y * blockDim.y + threadIdx.y;
	float val = 0.0;
...
void global_sgemm(float *C, float *A, float *B, long size)
{
	struct timeval t0, t1;
	gettimeofday(&t0, NULL);
	global_sgemm_kernel<<<grid, block>>>(C, A, B, size);
	checkCudaErrors(cudaPeekAtLastError());
	checkCudaErrors(cudaDeviceSynchronize());
	gettimeofday(&t1, NULL);
...
int main(int argc, char *argv[])
{
	...
	checkCudaErrors(cudaMemcpy(C_result, d_C, sizeof(float)*size*size, cudaMemcpyDeviceToHost));
	if (verify) compare_matrix(C_result, C_truth, size, THRESHOLD);
